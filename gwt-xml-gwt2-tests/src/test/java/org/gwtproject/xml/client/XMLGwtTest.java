...
import org.gwtproject.xml.client.impl.DOMParseException;

/** This class poorly tests all the methods in the GWT XML parser. */
public class XMLGwtTest extends GWTTestCase {
  private static List<Node> asList(Node[] nodes) {
    return Arrays.asList(nodes);
  }

  private static void assertAttributeMapEquals(NamedNodeMap listA, NamedNodeMap listB) {
  ...
  private static void assertCharacterDataEquals(CharacterData charDataA, CharacterData charDataB) {
  ...
  private static void assertNodeEquals(Node nodeA, Node nodeB) {
    ...
    switch (typeA) {
      ...
      case Node.PROCESSING_INSTRUCTION_NODE:
        assertProcessingInstructionEquals(
            (ProcessingInstruction) nodeA, (ProcessingInstruction) nodeB);
        break;
      default:
  ...
  public String getModuleName() {
    return "org.gwtproject.xml.XMLTest";
  }
  ...
  public void testDocument() {
    ...
    if (e1NodeDirect != null) {
      ...
      assertEquals(
          "getElementId returned unexpected element for XML node",
          e1Node.toString(),
          e1NodeDirect.toString());
  ...
  public void testNavigation() {
    ...
    assertEquals(
        "getPreviousSibling", documentElement.getPreviousSibling(), d.getChildNodes().item(0));
    ...
    assertEquals(
        "getFirstChild", documentElement.getFirstChild(), documentElementChildNodes.item(0));
    assertEquals(
        "getLastChild",
        documentElement.getLastChild(),
  ...
  /**
   * At one point, this test was failing on one Safari configuration in Production Mode in the 1.5
   * release branch.
   */
  public void testParse() {
    Document docA =
        XMLParser.parse("<!--hello-->   <a spam=\"ham\">\n  <?pi hello ?>dfgdfg  <b/>\t</a>");
  ...
  public void testText() {
    Document d = createTestDocument();
    List<Node> textLikeNodes =
        Arrays.asList(
            new Node[] {d.createTextNode(""), d.createCDATASection(""), d.createComment("")});
    ...
    for (Iterator<Node> iter = textLikeNodes.iterator(); iter.hasNext(); ) {
      ...
      // CHECKSTYLE_OFF
      assertEquals(
          "initialLength type:" + textLike.getNodeType(), 30000 - 32, textLike.getLength());
    ...
    for (int i = 32; i < 29900; i += 100) {
      for (Iterator<Node> iter = textLikeNodes.iterator(); iter.hasNext(); ) {
        ...
        // CHECKSTYLE_OFF
        assertEquals(
            "substring type:" + textLike.getNodeType() + " count: " + i,
            b.substring(i, i + 100),
            textLike.substringData(i, 100));
        // CHECKSTYLE_ON
      }
    }
    for (Iterator<Node> iter = textLikeNodes.iterator(); iter.hasNext(); ) {
      ...
      // CHECKSTYLE_OFF
      assertEquals(
          "deleteLength type:" + textLike.getNodeType(),
          bTemp.length(),
          textLike.getData().length());
      assertEquals(
          "deleteEquals type:" + textLike.getNodeType(), bTemp.toString(), textLike.getData());
      // CHECKSTYLE_ON
      bTemp.setLength(0);
    }
    for (Iterator<Node> iter = textLikeNodes.iterator(); iter.hasNext(); ) {
      ...
      // CHECKSTYLE_OFF
      assertEquals(
          "replaceLength type:" + textLike.getNodeType(),
          bTemp.length(),
          textLike.getData().length());
      assertEquals(
          "replaceEquals type:" + textLike.getNodeType(), bTemp.toString(), textLike.getData());
      // CHECKSTYLE_ON
      bTemp.setLength(0);
    }
    for (Iterator<Node> iter = textLikeNodes.iterator(); iter.hasNext(); ) {
    ...
    CDATASection cd = (CDATASection) d.getDocumentElement().getChildNodes().item(7);
